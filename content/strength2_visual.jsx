  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0F1A", color: "#E2E8F0", fontFamily: "'DM Sans', sans-serif", padding: "40px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: 720, width: "100%", marginBottom: 28 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#64748B", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Strength 02 / 07</div>
      ...
      <div style={{ maxWidth: 720, width: "100%", marginBottom: 16 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 10, color: "#475569", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Choose a solved domain to map from</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          {SOLVED_STRUCTURES.map((s, i) => (
            <button key={s.id} onClick={() => handleDomainChange(i)} style={{ padding: "10px 16px", background: selectedDomain === i ? `${s.color}15` : "rgba(15,22,41,0.6)", border: `1px solid ${selectedDomain === i ? s.color + "50" : "rgba(148,163,184,0.08)"}`, borderRadius: 8, cursor: "pointer", display: "flex", alignItems: "center", gap: 8, transition: "all 0.3s ease" }}>
              ...
              <span style={{ fontFamily: "'DM Sans'", fontSize: 12.5, fontWeight: 500, color: selectedDomain === i ? s.color : "#94A3B8" }}>{s.label}</span>
            </button>
          ))}
        </div>
      </div>

      <div style={{ background: "linear-gradient(180deg, #0F1629 0%, #131B2E 100%)", borderRadius: 16, border: "1px solid rgba(148,163,184,0.08)", padding: "24px 20px 16px", maxWidth: 720, width: "100%", boxShadow: "0 4px 40px rgba(0,0,0,0.4)" }}>
        <svg viewBox={`0 0 ${W + padX * 2} ${H + padY * 2}`} style={{ width: "100%", height: "auto", display: "block" }}>
          ...
          <text x={padX + TRANSFER_ZONE_X * W} y={padY + 16} textAnchor="middle" fill="#64748B" fontSize="8" fontFamily="'JetBrains Mono', monospace" letterSpacing="0.15em" opacity={transferActive ? 1 : 0} style={{ transition: "opacity 0.5s ease 0.2s" }}>STRUCTURE TRANSFER</text>
          <text x={padX + 0.12 * W} y={padY + 12} textAnchor="middle" fill={solved.color} fontSize="9" fontFamily="'JetBrains Mono', monospace" fontWeight="500" letterSpacing="0.08em">{solved.label.toUpperCase()}</text>
          <text x={padX + 0.88 * W} y={padY + 12} textAnchor="middle" fill="#3B82F6" fontSize="9" fontFamily="'JetBrains Mono', monospace" fontWeight="500" letterSpacing="0.08em">STARTUP STRATEGY</text>
          ...
          {solved.nodes.map((node, i) => {
            ...
            return (
              <g key={`src-${i}`}>
                ...
                <text x={cx + 14} y={cy + 4} fill="#CBD5E1" fontSize="10.5" fontFamily="'DM Sans'" fontWeight="500">{node.label}</text>
              </g>
            );
          })}
          ...
          {mappings.map((label, i) => {
            ...
            return (
              <g key={`tgt-${i}`} opacity={showMappings ? 1 : 0} style={{ transition: `opacity 0.4s ease ${0.4 + i * 0.15}s` }}>
                ...
                <text x={cx - 14} y={cy + 4} textAnchor="end" fill="#CBD5E1" fontSize="10.5" fontFamily="'DM Sans'" fontWeight="500">{label}</text>
              </g>
            );
          })}
        </svg>
      ...
      <div style={{ maxWidth: 720, width: "100%", marginTop: 16, display: "flex", gap: 12, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 340px", padding: "18px 20px", background: `${solved.color}08`, border: `1px solid ${solved.color}20`, borderRadius: 12, opacity: showMappings ? 1 : 0, transform: showMappings ? "translateY(0)" : "translateY(8px)", transition: "all 0.5s ease" }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#475569", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Structure Map</div>
          {solved.nodes.map((node, i) => (
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: i < 3 ? 8 : 0, fontSize: 12.5, fontFamily: "'DM Sans'" }}>
        ...
        <div style={{ flex: "1 1 300px", padding: "18px 20px", background: "rgba(248,113,113,0.04)", border: "1px solid rgba(248,113,113,0.15)", borderRadius: 12, opacity: showBreakdown ? 1 : 0, transform: showBreakdown ? "translateY(0)" : "translateY(8px)", transition: "all 0.5s ease" }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#F87171", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>⚡ Where the analogy breaks</div>
          <div style={{ fontSize: 13, color: "#CBD5E1", lineHeight: 1.7, fontFamily: "'DM Sans'" }}>{breakdown}</div>
          <div style={{ fontSize: 11.5, color: "#64748B", lineHeight: 1.6, marginTop: 10, fontFamily: "'DM Sans'" }}>The breakdown reveals what makes your domain <em>structurally unique</em>.</div>
        </div>
      </div>
