...
const TIER_COLORS = { centroid: "#94A3B8", filtered: "#F59E0B", edge: "#3B82F6", novel: "#10B981" };
const TIER_LABELS = { centroid: "GENERIC", filtered: "FILTERED", edge: "DISTINCTIVE", novel: "NOVEL" };

export default function Strength3Visual() {
  const [activeConstraints, setActiveConstraints] = useState(0);
  const toggleConstraint = (idx) => { setActiveConstraints(idx < activeConstraints ? idx : idx + 1); };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0F1A", color: "#E2E8F0", fontFamily: "'DM Sans', sans-serif", padding: "40px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: 720, width: "100%", marginBottom: 28 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#64748B", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Strength 03 / 07</div>
        ...
        <p style={{ fontSize: 15, color: "#94A3B8", lineHeight: 1.65, margin: 0, maxWidth: 620 }}>Each constraint <strong style={{ color: "#CBD5E1" }}>eliminates a region of probability space</strong>. Add enough and the "safe, average" center becomes impossible — the model must find solutions at the creative edges. Like jazz: rigid structure <em>forces</em> creativity.</p>
      </div>

      {/* Probability Bar */}
      <div style={{ maxWidth: 720, width: "100%", marginBottom: 20 }}>
        <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#475569", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>Output probability space — {remainingArea}% remaining</div>
        <div style={{ width: "100%", height: 32, background: "rgba(15,22,41,0.6)", borderRadius: 8, border: "1px solid rgba(148,163,184,0.08)", display: "flex", overflow: "hidden" }}>
          {CONSTRAINTS.slice(0, activeConstraints).map((c) => (
            <div key={c.id} style={{ width: "16%", height: "100%", background: `${c.color}30`, borderRight: "1px solid rgba(0,0,0,0.3)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.4s ease" }}>
              <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 8, color: c.color }}>✕</span>
            </div>
          ))}
          <div style={{ flex: 1, height: "100%", background: activeConstraints >= 5 ? "linear-gradient(90deg, #10B98120, #10B98108)" : activeConstraints >= 3 ? "linear-gradient(90deg, #3B82F620, #3B82F608)" : "rgba(148,163,184,0.05)", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.4s ease" }}>
            <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, fontWeight: 500, letterSpacing: "0.08em", color: activeConstraints >= 5 ? "#10B981" : activeConstraints >= 3 ? "#3B82F6" : "#64748B", transition: "color 0.4s ease" }}>
      ...
      {/* Constraint Toggles */}
      <div style={{ maxWidth: 720, width: "100%", marginBottom: 20 }}>
        <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#475569", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Click constraints to add/remove</div>
          ...
          {CONSTRAINTS.map((c, i) => {
            ...
            return (
              <button key={c.id} onClick={() => toggleConstraint(i)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", background: isActive ? `${c.color}10` : "rgba(15,22,41,0.4)", border: `1px solid ${isActive ? c.color + "40" : "rgba(148,163,184,0.06)"}`, borderRadius: 8, cursor: "pointer", transition: "all 0.3s ease", textAlign: "left" }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 500, color: isActive ? c.color : "#94A3B8", fontFamily: "'DM Sans'" }}>{c.label}</div>
                  {isActive && <div style={{ fontSize: 11, color: "#64748B", marginTop: 2, fontFamily: "'DM Sans'" }}>Eliminates: {c.eliminates}</div>}
                </div>
                <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 9, padding: "3px 8px", borderRadius: 4, background: c.type === "hard" ? "rgba(59,130,246,0.1)" : c.type === "exclude" ? "rgba(232,69,60,0.1)" : "rgba(6,182,212,0.1)", color: c.type === "hard" ? "#60A5FA" : c.type === "exclude" ? "#F87171" : "#22D3EE", letterSpacing: "0.05em", textTransform: "uppercase" }}>{c.type}</div>
              </button>
            );
          })}
        </div>
      ...
      {/* Outputs */}
      <div style={{ maxWidth: 720, width: "100%", display: "flex", gap: 12, flexWrap: "wrap" }}>
        <div style={{ flex: "1 1 320px", padding: "16px 18px", background: "rgba(15,22,41,0.4)", border: "1px solid rgba(148,163,184,0.06)", borderRadius: 12 }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#EF4444", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10 }}>Eliminated (the clichés)</div>
          {OUTPUTS.filter(o => o.tier === "centroid").map((o, i) => (
            <div key={i} style={{ fontSize: 12.5, fontFamily: "'DM Sans'", padding: "5px 0", color: activeConstraints > 0 ? "#475569" : "#94A3B8", textDecoration: activeConstraints > 0 ? "line-through" : "none", transition: "all 0.3s ease" }}>{o.label}</div>
          ))}
        </div>
        <div style={{ flex: "1 1 320px", padding: "16px 18px", background: activeConstraints >= 4 ? "rgba(16,185,129,0.04)" : "rgba(15,22,41,0.4)", border: `1px solid ${activeConstraints >= 4 ? "rgba(16,185,129,0.15)" : "rgba(148,163,184,0.06)"}`, borderRadius: 12, transition: "all 0.4s ease" }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: activeConstraints >= 4 ? "#10B981" : "#64748B", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 10, transition: "color 0.3s ease" }}>
          ...
          {OUTPUTS.filter(o => o.tier !== "centroid").map((o, i) => {
            ...
            return (
              <div key={i} style={{ fontSize: 12.5, fontFamily: "'DM Sans'", padding: "5px 0", display: "flex", alignItems: "center", gap: 8, opacity: visible ? 1 : 0.15, transform: visible ? "translateX(0)" : "translateX(-4px)", transition: "all 0.4s ease" }}>
                ...
                <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 8, color: TIER_COLORS[o.tier], letterSpacing: "0.05em", marginLeft: "auto", opacity: visible ? 0.7 : 0 }}>{TIER_LABELS[o.tier]}</span>
              </div>
            );
          })}
        </div>
