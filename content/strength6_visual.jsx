  ...
  return (
    <div style={{ marginBottom: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
        <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#64748B", letterSpacing: "0.05em" }}>{label}</span>
        <span style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color, fontWeight: 500 }}>{value}%</span>
      </div>
...
export default function Strength6Visual() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0B0F1A", color: "#E2E8F0", fontFamily: "'DM Sans', sans-serif", padding: "40px 20px", display: "flex", flexDirection: "column", alignItems: "center" }}>
      ...
      <div style={{ maxWidth: 760, width: "100%", marginBottom: 28 }}>
        <div style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 11, color: "#64748B", letterSpacing: "0.12em", textTransform: "uppercase", marginBottom: 8 }}>Strength 06 / 07</div>
      ...
      {/* Mode Selector */}
      <div style={{ maxWidth: 760, width: "100%", marginBottom: 16, display: "flex", gap: 6, flexWrap: "wrap" }}>
        {MODES.map((m, i) => (
      ...
      {/* Prompt */}
      <div style={{ maxWidth: 760, width: "100%", marginBottom: 12, padding: "12px 16px", background: `${mode.color}06`, border: `1px solid ${mode.color}15`, borderRadius: 10 }}>
        ...
        <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 11, color: mode.color, lineHeight: 1.6, whiteSpace: "pre-wrap" }}>{mode.prompt}</div>
      </div>

      <div style={{ maxWidth: 760, width: "100%", display: "flex", gap: 12, flexWrap: "wrap" }}>
        {/* Output */}
        <div style={{ flex: "1 1 440px", padding: "18px 20px", background: `${mode.color}06`, border: `1px solid ${mode.color}18`, borderRadius: 14, minHeight: 260 }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: mode.color, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: 10 }}>Output</div>
          ...
          {mode.table && (
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 11.5 }}>
                <thead>
                  <tr>{mode.table.headers.map((h, i) => <th key={i} style={{ padding: "6px 8px", textAlign: "left", color: "#64748B", fontFamily: "'JetBrains Mono'", fontSize: 9, borderBottom: "1px solid rgba(148,163,184,0.1)", letterSpacing: "0.05em" }}>{h}</th>)}</tr>
                </thead>
                <tbody>
                  {mode.table.rows.map((row, ri) => <tr key={ri}>{row.map((cell, ci) => <td key={ci} style={{ padding: "6px 8px", color: "#CBD5E1", borderBottom: "1px solid rgba(148,163,184,0.05)", fontFamily: "'DM Sans'" }}>{cell}</td>)}</tr>)}
                </tbody>
              </table>
            </div>
          )}
          ...
          {mode.schema && (
            <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 11, lineHeight: 1.7 }}>
        ...
        <div style={{ flex: "0 0 220px", padding: "18px 20px", background: "linear-gradient(180deg, #0F1629, #131B2E)", border: "1px solid rgba(148,163,184,0.08)", borderRadius: 14 }}>
          <div style={{ fontFamily: "'JetBrains Mono'", fontSize: 10, color: "#475569", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 14 }}>Quality scores</div>
          ...
          <div style={{ marginTop: 16, padding: "10px 12px", background: `${mode.color}08`, borderRadius: 6, fontSize: 11.5, color: "#94A3B8", lineHeight: 1.5, fontFamily: "'DM Sans'" }}>
            {mode.verdict}
          </div>
        </div>
      </div>
      ...
      {/* DB Analogy */}
      <div style={{ maxWidth: 760, width: "100%", marginTop: 16, padding: "14px 20px", background: "rgba(15,22,41,0.5)", border: "1px solid rgba(148,163,184,0.06)", borderRadius: 10 }}>
